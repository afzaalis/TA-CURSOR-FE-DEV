 ...
 * ─────────────────────
 * Ganti sumber data: halaman ini memakai prop `data` (lihat tipe PropertyDetailPageData).
 * Alur produksi yang disarankan:
 * 1) Buat route dinamis `app/detail-properti/[id]/page.tsx` sebagai Server Component.
 * 2) Panggil API / DB di server: `const data = await getPropertyById(id)`.
 * 3) Map respons backend ke bentuk PropertyDetailPageData (atau pisahkan mapper di lib/).
 * 4) Render `<DetailPropertyPageKontekstual data={mapped} />` tanpa mengubah markup di dalam komponen ini.
 * Untuk preview lokal, gunakan konstanta MOCK_DETAIL di bawah atau di page wrapper.
 */
import Image from "next/image";
import { Poppins } from "next/font/google";
import { useState } from "react";
import { Bath, BedDouble, ChevronLeft, ChevronRight } from "lucide-react";
...
export type PropertyDetailSpecRow = {
  label: string;
  value: string;
};
...
export type PropertyDetailAgent = {
  ...
  photoSrc: string;
  waUrl: string;
  /** Opsional, mis. nama perusahaan agen */
  company?: string;
};
...
export type PropertyDetailPageData = {
  ...
  bedrooms: number;
  bathrooms: number;
  /** Angka atau teks tampilan untuk kolom m² ringkas di header (contoh: "120") */
  landAreaDisplay: string;
  description: string;
  specifications: PropertyDetailSpecRow[];
  about: PropertyDetailSpecRow[];
  interior: PropertyDetailSpecRow[];
  facilitiesHome: string[];
  facilitiesCluster: string[];
  /** Minimal 3 URL untuk pola thumbnail (2 + Selengkapnya) */
  gallery: string[];
  mapEmbedUrl: string;
  agent: PropertyDetailAgent;
  relatedListings: PropertyData[];
};

export const MOCK_DETAIL: PropertyDetailPageData = {
  title: "Perumahan Cherry Field Tipe A",
  price: 850_000_000,
  location: {
    city: "Bandung",
    district: "Bojongsoang",
    developer: "Ted. Estate Indonesia",
  },
  bedrooms: 3,
  bathrooms: 2,
  landAreaDisplay: "120",
  description:
    "Rumah siap huni di kawasan strategis dengan akses mudah ke jalan utama dan fasilitas umum. Lingkungan asri, one gate system, serta taman bermain untuk keluarga. Cocok untuk pasangan muda maupun investasi jangka panjang dengan potensi kenaikan nilai aset yang stabil di wilayah ini.",
  specifications: [
    ...
    { label: "Kondisi properti", value: "Baru" },
  ],
  about: [
    { label: "Lebar jalan", value: "6 m" },
    { label: "Hook", value: "Tidak" },
    { label: "Tahun bangun", value: "2022" },
    { label: "Tipe properti", value: "Rumah" },
    { label: "Tipe iklan", value: "Dijual" },
    { label: "ID iklan", value: "SR-2024-08921" },
  ],
  interior: [
    { label: "Kondisi", value: "Bagus" },
    { label: "Perabotan", value: "Tidak termasuk" },
    { label: "Peralatan", value: "Partial" },
    { label: "Hadap", value: "Timur" },
    { label: "Konsep & gaya rumah", value: "Modern minimalis" },
  ],
  facilitiesHome: ["Listrik PLN", "Air PDAM"],
  facilitiesCluster: [
    "One gate",
    "Taman bermain",
    "Security 24 jam",
  ...
  mapEmbedUrl:
    "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3960.349335739937!2d107.629127!3d-6.917464!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x2e68e7c9b8b8b8b9%3A0x1!2sBandung!5e0!3m2!1sid!2sid!4v1",
  agent: {
    name: "Lestari Najwa",
    photoSrc: "/images/logo.png",
    waUrl: "https://wa.me/6280000000000",
    company: "PT. Sinar Mas",
  },
  relatedListings: Array.from({ length: 6 }, (_, i) => ({
    id: `rel-ctx-${i + 1}`,
    isPromoted: true,
...
function SpecItem({ label, value }: PropertyDetailSpecRow) {
  return (
    ...
    >
      <div className="mb-4 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="font-bold text-[#1E293B]">{agent.name}</p>
          {agent.company ? (
...
export type DetailPropertyPageKontekstualProps = {
  data: PropertyDetailPageData;
};

export default function DetailPropertyPageKontekstual({
  data,
}: DetailPropertyPageKontekstualProps) {
  const [activeImage, setActiveImage] = useState(0);

  const gallery = (() => {
    const g = [...data.gallery];
    if (g.length === 0) {
      return ["/images/logo.png", "/images/logo.png", "/images/logo.png"];
    }
    while (g.length < 3) {
      g.push(g[g.length - 1] ?? g[0]);
    }
    return g;
  })();

  const addressLine = `${data.location.district}, ${data.location.city}`;
    ...
    >
      ...
      <main className="flex-1">
        <div className="mx-auto max-w-[1440px] px-5 py-8 lg:px-10">
          <section className="mb-8 lg:mb-10" aria-label="Galeri foto properti">
          ...
          <div className="mb-8 lg:mb-10 lg:grid lg:grid-cols-[1fr_min(360px,100%)] lg:items-start lg:gap-10">
            <div className="min-w-0 space-y-8">
              <section className="py-1">
                <p className="mb-4 text-2xl font-bold italic text-[#1957A0] sm:mb-5 sm:text-[26px]">
                  {formatPropertyPriceIdr(data.price)}
                </p>
                <div className="flex flex-col gap-8 sm:flex-row sm:items-stretch sm:justify-between sm:gap-10">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-sm text-[#64748B]">{addressLine}</p>
              ...
              <AgentPropertyCard agent={data.agent} className="lg:hidden" />
              ...
              <div className="space-y-10 py-2">
                ...
                <section>
                  ...
                  <div className="max-w-2xl">
                    {data.specifications.map((row, i) => (
                      <SpecItem key={`spec-${i}-${row.label}`} {...row} />
                    ))}
                  </div>
                </section>
                ...
                <section>
                  ...
                  <div className="max-w-2xl">
                    {data.about.map((row, i) => (
                      <SpecItem key={`about-${i}-${row.label}`} {...row} />
                    ))}
                  </div>
                </section>
                ...
                <section>
                  ...
                  <div className="max-w-2xl">
                    {data.interior.map((row, i) => (
                      <SpecItem key={`int-${i}-${row.label}`} {...row} />
                    ))}
                  </div>
                </section>
                ...
                <section>
                  ...
                  <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4">
                    {data.facilitiesCluster.map((label) => (
                      <FacilityItem key={label} label={label} />
                    ))}
                  </div>
                </section>
            ...
            >
              <div className="sticky top-24 z-10 pb-8 pt-0">
                <AgentPropertyCard agent={data.agent} />
              </div>
            </aside>
          </div>
        ...
        <div className="mx-auto max-w-[1440px] px-5 py-12 lg:px-10">
          <section>
            <div className="mb-6 flex items-center justify-between">
              <h2
                id="perumahan-populer-heading-kontekstual"
